...
3 7 8 5 3 6 0 4 5 6
    */
    ...
    snake_coord = vector<vector<pair<int, int>>>(S);
    map_snakes = vector<vector<string>>(R, vector<string>(C));

    for (int i = 0; i < S; i++) {
        cin >> snake_length[i];
        snake_coord[i] = vector<pair<int, int>>(snake_length[i]*2);
    }
...
pair<int,int> get_next_cell(int x, int y){ //look for best cell in neighbours
    int best_x;
    int best_y;
    int best_val = -1;
    ...
    for (int k = -1; k <= 1; k++) {
        for (int l = -1; l <= 1; l++) {
            ...
            if (map[new_x][new_y] != "*" && map_snakes[new_x][new_y] == EMPTY) {
                if (stoi(map[new_x][new_y]) > best_val) {
                    best_val = stoi(map[new_x][new_y]);
                    best_x = new_x;
                    best_y = new_y;
                }
            }
    ...
    //if no best cell found, and warmhole as neiboour, teleport to the first next warmhole found
    if(best_val == -1){
        for (int k = -1; k <= 1; k++) {
            for (int l = -1; l <= 1; l++) {
                //exclude the diagonals too and same cell
                            ...
                            if (map[m][n] == "*") {
                                ...
                                for (int o = -1; o <= 1; o++) {
                                    for (int p = -1; p <= 1; p++) {
                                        ...
                                        int new_n = (n + p + C) % C;
                                        if (map[new_m][new_n] != "*" && map_snakes[new_m][new_n] == EMPTY) {
                                            if (stoi(map[new_m][new_n]) > best_val) {
                                                best_val = stoi(map[new_m][new_n]);
                                                best_x = new_m;
                                                best_y = new_n;
                                            }
                                        }
    ...
    //if no best cell found, and no warmhole as neiboour, return the same cell
    if(best_val == -1){
        return {x, y};
    }
...
void find_solution_basic(){
    //find a solution positioning the snakes in the map, cell by cell, without looking for the best cell, if encouter a warmhole, just teleport to the first other warmhole found
    int snake_index = 0;
    for(int i=0; i<C; i++){
        int j=0;
        //position the head of the snake
        while(map[j][i] == "*" || map_snakes[j][i] != EMPTY){
            j++;
        }
        snake_coord[snake_index][0] = {j,i};
        //position the body of the snake, if warmhole, teleport to the first warmhole found
        int snake_len = 1;
        while(snake_len < snake_length[snake_index]){
            //use get_next_cell
            pair<int,int> next_cell = get_next_cell(j,i);
            if(next_cell.first == j && next_cell.second == i){
                snake_length[snake_index] = 0;
                break;
            }
            //if next cell is warmhole, add also start warmhole and end warmhole
            snake_coord[snake_index][snake_len++] = {j,i};
            snake_coord[snake_index][snake_len++] = next_cell;
        ...
        for (int j = 1; j < snake_length[i]; j++) {
            ...
            if (snake_coord[i][j].first == snake_coord[i][j - 1].first) {
                ...
                if (snake_coord[i][j].first == (snake_coord[i][j - 1].first + 1) % R) {
                    cout << "D ";
                } else if (snake_coord[i][j].first == (snake_coord[i][j - 1].first - 1 + R) % R) {
                    cout << "U ";
                }
            }
...
int main() {
    ...
    //print map_snakes format a tab for each cell adding the numbers of row and column
    for(int i = 0; i < C; i++){
        cout << "\t" << i;
    }
    cout << endl;
