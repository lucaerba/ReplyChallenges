...
void write_output() {
    ...
    */
    for (int i = 0; i < S; i++) {
     /*    if(i == 9) {
            cout << "PRINTING SNAKE " << i << " LENGHT " << snake_length[i] << " COORDS " << snake_coord[i].size() << endl;

            // print all the vectors snake_coord
            for (int j = 0; j < snake_length[i]; j++) {
                cout << snake_coord[i][j].first << " " << snake_coord[i][j].second << " ";
            }
        } */
        if (snake_length[i] == 0) {
            cout << endl;
        ...
        for (int j = 1; j < snake_length[i]; j++) {
            ...
            if (snake_coord[i][j].first == snake_coord[i][j - 1].first) {
                ...
                if (snake_coord[i][j].first == (snake_coord[i][j - 1].first + 1) % R) {
                    ...
                    cout << "ERROR : " << snake_length[i] << " " << j << " " << snake_coord[i][j].first << " " << snake_coord[i][j].second << " " << snake_coord[i][j - 1].first << " " << snake_coord[i][j - 1].second << " ";
                }
            }
